"use strict";
// pre-defined products
var AJ = { name: "Air Jordans", price: 1000, img: "https://images.unsplash.com/photo-1610870372593-a8647b04451f?ixid=MnwxMjA3fDB8MHxzZWFyY2h8Mnx8bmlrZSUyMGpvcmRhbnxlbnwwfHwwfHw%3D&ixlib=rb-1.2.1&auto=format&fit=crop&w=500&q=60" };
var Vans = { name: "Vans", price: 250, img: "https://images.unsplash.com/photo-1525966222134-fcfa99b8ae77?ixlib=rb-1.2.1&ixid=MnwxMjA3fDB8MHxzZWFyY2h8NHx8dmFuc3xlbnwwfHwwfHw%3D&auto=format&fit=crop&w=500&q=60" };
var Conv = { name: "Converse ", price: 100, img: "https://images.unsplash.com/photo-1607522370275-f14206abe5d3?ixid=MnwxMjA3fDB8MHxwaG90by1wYWdlfHx8fGVufDB8fHx8&ixlib=rb-1.2.1&auto=format&fit=crop&w=1008&q=80" };
var Curry = { name: "Curry 8", price: 450, img: "https://images.unsplash.com/photo-1491553895911-0055eca6402d?ixlib=rb-1.2.1&ixid=MnwxMjA3fDB8MHxzZWFyY2h8MXx8c2hvZXN8ZW58MHx8MHx8&auto=format&fit=crop&w=500&q=60" };
var Addidas = { name: "Superstars", price: 500, img: "https://images.unsplash.com/photo-1551116198-01d550c9809c?ixid=MnwxMjA3fDB8MHxzZWFyY2h8MXx8YWRkaWRhc3xlbnwwfHwwfHw%3D&ixlib=rb-1.2.1&auto=format&fit=crop&w=500&q=60" };
var shoes = new Array(AJ, Vans, Conv, Curry, Addidas);
// empty cart
var cart = new Array();
function displayShoes() {
    var _loop_1 = function (item) {
        var name_1 = "Name :" + item.name;
        var price = "Price:" + "$ " + item.price.toLocaleString("USD");
        var divTag = document.createElement("div");
        divTag.setAttribute("class", "col-sm-6 col-md-4");
        //card
        var cardTag = document.createElement("div");
        cardTag.setAttribute("class", "card");
        //card top
        var imgTag = document.createElement("img");
        imgTag.setAttribute("class", "card-img-top");
        if (item.img) {
            imgTag.setAttribute("src", item.img);
        }
        else {
            imgTag.setAttribute("src", "data:image/jpeg;base64,");
        }
        imgTag.setAttribute("style", "height:225px; width:100%; display:block");
        //card body part
        var cardBodyTag = document.createElement("div");
        cardBodyTag.setAttribute("class", "card-body");
        var nameTag = document.createElement("h5");
        nameTag.setAttribute("class", "card-title");
        nameTag.appendChild(document.createTextNode(name_1));
        var priceTag = document.createElement("p");
        priceTag.setAttribute("class", "card-text");
        priceTag.appendChild(document.createTextNode(price));
        var addButton = document.createElement("input");
        addButton.setAttribute("type", "button");
        addButton.setAttribute("value", "Add to Cart");
        addButton.setAttribute("class", "btn btn-primary");
        addButton.addEventListener("click", function () { return addToCart(item); });
        cardBodyTag.appendChild(nameTag);
        cardBodyTag.appendChild(priceTag);
        //adding content to card
        cardTag.appendChild(imgTag);
        cardTag.appendChild(cardBodyTag);
        cardTag.appendChild(addButton);
        divTag.appendChild(cardTag);
        // refer the tag using id selector and append the p tag to div tag
        document.getElementById("items").appendChild(divTag);
    };
    for (var _i = 0, shoes_1 = shoes; _i < shoes_1.length; _i++) {
        var item = shoes_1[_i];
        _loop_1(item);
    }
}
function addToCart(item) {
    cart.push(item);
    localStorage.setItem("cartObj", JSON.stringify(cart));
    document.getElementById("size").innerText = cart.length + "";
}
function displayCart() {
    var cartObj = localStorage.getItem("cartObj") || "{}";
    var cartJson = JSON.parse(cartObj);
    var tableContent = "";
    var startTable = "<table class = table table-striped><thead><tr><th scope = col>Item Name</th><th th scope = col>Price</th></tr></thead>";
    var total = 0;
    for (var i = 0; i < cartJson.length; i++) {
        var obj = cartJson[i];
        var priceStr = "$ " + Number(obj.price).toLocaleString("USD");
        tableContent += "<tr><td>" + obj.name
            + "</td><td>" + priceStr + "</td></tr>";
        total = total + obj.price;
    }
    //display table
    var endTable = "</table>";
    tableContent = startTable + tableContent + endTable;
    var cartTable = document.getElementById("cartTable");
    if (cartTable)
        cartTable.innerHTML = tableContent;
    //display total
    var totalStr = "$ " + Number(total).toLocaleString("USD");
    var totalRow = "<b>" + totalStr + "</b>";
    document.getElementById("total_price").innerHTML = totalRow;
}
function addalert() {
    alert("Thank you for shopping with us");
}
